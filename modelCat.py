...
def k_fold(X, y, model, k=10, random_state=6):
    ...
    scores = []
    predictions = pd.DataFrame(columns=['PassengerId', 'Transported'])
    for train_index, val_index in kf.split(X):
        ...
        try:
            ...
            predictions_subset = pd.DataFrame(columns=["PassengerId", 'Transported'])
        except Exception as e:
            logging.error(f"Failed to fit the model: {e}")
...
logging.info(f'Model start trainng')
...
logging.info(f'CatBoost accuracy: {score}')
